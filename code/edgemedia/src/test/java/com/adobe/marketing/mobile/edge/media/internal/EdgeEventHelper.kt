    ...
    companion object {
        ...
        private fun getSessionDetails(
            ...
            val details: MutableMap<String, Any> = mutableMapOf(
                ...
                "length" to ((info?.get("media.length") as? Double) ?: -1),
        ...
        private fun getAdvertisingDetails(
            ...
            val details: MutableMap<String, Any> = mutableMapOf(
                ...
                "length" to ((info?.get("ad.length") as? Double) ?: -1)
            )
        ...
        private fun getAdvertisingPodDetails(info: Map<String, Any>?): Map<String, Any> {
            return mapOf(
                ...
                "offset" to ((info?.get("adbreak.starttime") as? Double) ?: -1)
            )
        }
        ...
        private fun getChapterDetails(info: Map<String, Any>?): Map<String, Any> {
            return mapOf(
                ...
                "offset" to ((info?.get("chapter.starttime") as? Double) ?: -1),
                "length" to ((info?.get("chapter.length") as? Double) ?: -1)
            )
        }
        ...
        private fun getQoeDetails(info: Map<String, Any>?): Map<String, Any> {
            return mapOf(
                "bitrate" to ((info?.get("qoe.bitrate") as? Double) ?: -1),
                "droppedFrames" to ((info?.get("qoe.droppedframes") as? Double) ?: -1),
                "framesPerSecond" to ((info?.get("qoe.fps") as? Double) ?: -1),
                "timeToStart" to ((info?.get("qoe.startuptime") as? Double) ?: -1)
            )
        }
    }
}
